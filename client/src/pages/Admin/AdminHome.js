...
const AdminHome = () => {
  const { user } = useSelector((state) => state.auth); // Accessing user object from Redux store

  return (
    <Layout> {/* Rendering Layout component for consistent page structure */}
      <div className="container">
        <div className="d-felx flex-column mt-4"> {/* Container for content */}
          <h1>
            Welcome Admin <i className="text-success">{user?.name}</i> {/* Displaying admin's name */}
          </h1>
          <h3>Manage Blood Bank App </h3> {/* Subheading */}
          <hr /> {/* Horizontal line */}
          <p>
